...
_Jon_: You know what? It's actually not too bad. It's actually not too
...
_Ben_: At one point Rust itself was a side project. So who knows what things
might, you know, rise from the ashes?

_Jon_: That's very true.

_Ben_: And fortunately, Rust development has always been highly distributed
and remote heavy, and so it continues apace.

_Jon_: It's true, and one other upside of it being a while since last time we
recorded is that we now get to record for not just one, but two Rust releases.
Yeah, 1.42 and 1.43.

_Ben_: Let's jump into it. I'm not sure how long it will take me but we'll
...
_Jon_: So, 1.42 was a pretty cool release, actually. I thought there were a
...
_Ben_: This is kind of, it's it's somewhat of a revelation. It's one of those
...
_Jon_: Yeah, I know, right? Or like, I always have `RUST_BACKTRACE` set to
...
_Ben_: Yeah. Do you want to talk more about how the compiler achieves this
and whether or not users can use this in their own code?

_Jon_: Yeah. So this actually required some kind of subtle changes internally
...
_Ben_: So currently, the standard library uses this mechanism internally. And
...
_Jon_: Yeah, there's another cool change in 1.42 which is this this notion
of subslice patterns and and I hear that you maybe had a hand in this, Ben.

_Ben_: I don't know about hand. So just like the previous point, this one has
...
_Jon_: So I was going to say, like, so some might argue that this is, like,
extending the syntax, right?  Like you're adding more complex syntax to Rust.
Do you feel like that's true about this feature?

_Ben_: No, actually, I think this is a case of— I judge language features
...
_Jon_: So what is the subslice part of this feature?

_Ben_: Basically, in other languages, so in, like, a functional language,
...
_Jon_: Well, specifically, bind it to this name if it matches.

_Ben_: So, for example, if I have, like, an array of `1, 2, 3`, of numbers,
...
_Jon_: Yeah, I feel like it's it's really neat, right? In some sense it ends
up being a very unsurprising syntax, I think.

_Ben_: Yeah, it's just— we're composing elements of the syntax that already
...
_Jon_: Right. And ideally, I guess you would want it to mirror the syntax
that's now been introduced for matching them.

_Ben_: Yeah, and so— but again, this is not new syntax. All the syntax
...
_Jon_: I think this is sort of how people try to emulate something like
named function arguments, right?

_Ben_: Yeah. And it's often used in combination with the default trait to
kind of get, like, default documents, too.

_Jon_: Yeah.

_Ben_: So there's some question here. If you want to have dedicated syntax for
...
_Jon_: I don't know. I'm pretty happy with the syntax.

_Ben_: I think it works, yeah.

_Jon_: So that clearly settles it.

_Ben_: I'm just happy that we can do it now and that we haven't let syntax...

_Jon_: Yeah, I think that's true.

_Ben_: ... block progress any longer where it's like, it's nice to be able
...
_Ben_: So, learn to use @-patterns. They're really cool.

_Jon_: Yeah, they are really handy.

_Jon_: Speaking of patterns, the next thing that comes up here is something
...
_Ben_: You could write it yourself, perhaps.

_Jon_: I actually proposed it in the PR that added `matches!` and the response
...
_Ben_: Now, speaking of additions, this is the opposite: deprecation. And
...
_Jon_: Yeah. So this is actually kind of interesting. So this is one of the...
...
_Ben_: Yeah, I think it's interesting to review how the Rust error-handling
...
_Jon_: I think it's really cool to see what's happening in the error space in
...
_Ben_: I want it back up. I haven't heard of this Jane author before. We're
...
_Jon_: Yeah, this is Jane Lusby. She does a lot of, like, really cool, Rust
sort of experimentation. And one of them is this `eyre` library, "e-y-r-e".

_Ben_: Jane Eyre is a famous novel.

_Jon_: Yeah, I know. It is really funny.

_Ben_: OK, I wasn't sure.

_Jon_: No, it's really fun.

_Ben_: So, awesome. Okay, and that, I think there are also a few other smaller
library things in 1.42 which, I don't think are very interesting. I'm not
sure if you want to talk about any of them.

_Jon_: No, I think I think these are relatively minor ones this time around. I
...
_Ben_: Cool. The only other thing I see here is downgrading 32-bit Apple
...
_Jon_: I don't think you can get 32-bit Apple things any more.

_Ben_: I thought they made the switch to 64 bit back in, like, the
mid-2000s. But maybe I'm misremembering.  Maybe that was the Intel switch. I
don't know.

_Jon_: Yeah. No, I think you're entirely right.

_Ben_: I'm sure someone out there is, and I'm sure that they are, you know,
...
_Ben_: But it doesn't mean that other platforms don't work. I believe that
...
_Jon_: Yeah, no, I think this seems like a very reasonable change, where
...
_Ben_: Yeah. We're actually not too far behind on this one.

_Jon_: Wow. Look at us go.

_Ben_: As of time of recording.

_Jon_: Yes, that's true. You're not wrong.
...
_Ben_: Well, I think, I'm not saying worried. I think that's too strong. I
...
_Jon_: I think it's a good point that this does become it becomes a part of
the Rust specification in some sense.

_Ben_: Yeah.

_Jon_: ... that if you wrote, like, if you wrote your own Rust compiler,
it would also need to support this other syntax because otherwise there
would be programs that would not work.

_Ben_: So rather than saying, I'm worried about it. It's not that. It's
...
_Ben_: Okay. I don't want to talk too much, because I'm not the expert
...
_Ben_: So yeah, that's it.

_Jon_: I mean, I think it's a good change, and it's interesting because this
...
_Jon_: And Steve Klabnik wrote a really interesting blog post recently where
...
_Ben_: In the toolchain too, with cargo and all these things.

_Jon_: Yeah, exactly. And I think we see this in the changelog too,
...
_Ben_: Speaking of tiny changes, the toolchain, I think you want to talk a
little bit about this cargo environment variable for tests.

_Jon_: Oh, yeah. So this change is is kind of funny because it's something
...
_Ben_: I was gonna say, like I was curious, if you could use `cargo run`
...
_Jon_: You can. There are actually crates to do this. And there's even a
crate for invoking cargo from inside of tests. There some reasons why you
don't really want to do this—

_Ben_: I can imagine.

_Jon_: Because, yeah, like, for example, what do you do with the output, right?
...
_Ben_: And this is not the first environment variable that cargo sets, right?

_Jon_: Yeah, that's true.  In fact, cargo sets lots of environment
...
_Ben_: So moving on to the library changes, I actually do have a thing to
...
_Jon_: I see. Because `u32` and `f32` there are modules as supposed to the
primitive type.

_Ben_: Yes. Modules, if actually— say you have to actually bring them up,
...
_Jon_: Do you think that the old versions are gonna end up being deprecated?

_Ben_: That was one of the big questions of the RFC that we eventually kind
...
_Ben_: And so the idea is that— I think the final decision was, yes,
...
_Jon_: I mean, I guess you can also do sort of the same thing they did with
...
_Ben_: I think that even in this case, `Error::description` is not actually
becoming a hard error, I don't believe, it's just deprecated.

_Jon_: No, it's just depreciated for now. But I think the intention,
presumably, is for the next edition to make it— a hard error, maybe?

_Ben_: That's actually— I think there is— that's still up in the air. I
...
_Jon_: Yeah, it would almost be like a lint that becomes deny.

_Ben_: But I don't think there's ever actually any precedent for doing this,
...
_Jon_: Yeah, I mean, it's a good question, right? This ties back to "what
...
_Ben_: No, it never goes away. And again, because it goes with never goes away,
...
_Jon_: Yeah. And I mean that we saw this with with `Error::cause` and
...
_Ben_: Wouldn't be compatible.

_Jon_: There are actually a few other things that I want to mention here. What
...
_Ben_: I do kind of do like this.

_Jon_: Yeah. I mean, it seems like a sort of weird change, but if you think
...
_Ben_: Yeah, well also, I don't even use macros. I mean, I don't even write
...
_Jon_: Yeah, I think that's right.

_Ben_: But as far as the users are concerned, it's the same thing.

_Jon_: So, yeah, it's funny you mentioned the prelude because that's another
...
_Ben_: Yeah, we'll find that link that for the show notes. And I think now
...
_Ben_: As opposed to the previous edition, which was kind of, like, just at
...
_Jon_: Or forever hold your peace.

_Ben_: Or I guess until the next edition comes around. I wouldn't say forever.
For three years, hold your peace.

_Jon_: There are a couple of other, like, smaller things I want to bring
...
_Ben_: I think it's kind of like, goes to show too— I think in Steve's blog
...
_Jon_: I think as with every version, I also want to touch briefly on, like,
...
_Ben_: And I'm wondering if that might be the kind of thing that might, even
...
_Jon_: That would be my guess. This new resolver fixes a bunch of other
...
_Ben_: And to get clippy, it doesn't come by default with rustup; I think
you need to do `rustup add component clippy`, right?

_Jon_: I forget. This might actually have changed with the the rustup profile
...
_Ben_: No, I'm good. So I think we're about ready to wrap this up.

_Jon_: Nice. Do you— are there other RustFest interviews coming? I'm
forgetting whether we have more.

_Ben_: I have one currently in the works that may or may not be— it probably
...
_Jon_: Yeah, they'll be good. That sounds like a great idea.

_Ben_: ... interview Niko Matsakis, maybe Felix Klock. So yeah, keep an eye
out for those.

_Jon_: Yeah, I think that would be really cool. And a reminder, of course,
...
_Ben_: All right, well, it's been good hearing from you again, Jon. Let's
do it again before too long.

_Jon_: Absolutely, Ben, Likewise. I'll see you soon.
